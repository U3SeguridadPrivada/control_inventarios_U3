'use client';
// Full Salidas page — identical logic to apps/web/src/pages/Salidas.tsx
// with import paths updated to @/src/*
...
import { ARTICULOS, CONCEPTOS_SALIDA, getEstadoAsignacion, requiereTalla as articuloRequiereTalla } from '@/src/lib/constants';
...
export default function SalidasPage() {
  ...
  const articulosSimple = useMemo(() => ARTICULOS.filter(art => (stockPorArticulo[estadoFisico][art] ?? 0) > 0), [stockPorArticulo, estadoFisico]);
              ...
              : filteredData.map((item: any) => {
                ...
                return (
                  <TableRow key={item.id}>
                    ...
                    <TableCell><span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium whitespace-nowrap ${cls}`}>{label}</span></TableCell>
                  </TableRow>
                );
              })}
          </TableBody>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5"><label className="text-sm font-medium">Artículo</label>
                  <Select value={articulo} onChange={e => setArticulo(e.target.value)} required>
                    ...
                    {articulosSimple.map(art => <option key={art} value={art}>{art} — Disp: {stockPorArticulo[estadoFisico][art] ?? 0}</option>)}
                  </Select>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
