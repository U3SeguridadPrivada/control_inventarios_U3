...
export default function GuardiasPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['guardias'] }); queryClient.invalidateQueries({ queryKey: ['dashboardMetrics'] }); toast.success('Guardia registrado'); setIsModalOpen(false); setNumeroElemento(''); setNombre(''); },
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden shadow-sm">
        <Table>
          <TableHeader><TableRow><TableHead>Número de Elemento</TableHead><TableHead>Nombre del Guardia</TableHead><TableHead>Fecha Alta</TableHead><TableHead>Estado Actual</TableHead><TableHead className="text-right">Acciones</TableHead></TableRow></TableHeader>
          <TableBody>
              ...
              : filteredData.map((item: any) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="sm" className="h-8 mr-2" onClick={() => { setSelectedGuardia(item); setIsExpedienteModalOpen(true); }}>Ver Expediente</Button>
                    {isEditor && item.estado === 'Activo' && <Button variant="outline" size="sm" className="h-8" onClick={() => { setSelectedGuardia(item); setIsBajaModalOpen(true); }}><LogOut className="w-3.5 h-3.5 mr-1.5" /> Dar de Baja</Button>}
                  </TableCell>
                </TableRow>
              ))}
          </TableBody>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent>
          <form onSubmit={e => { e.preventDefault(); createMutation.mutate({ numero_elemento: numeroElemento, nombre, fecha_alta: fechaAlta }); }}>
            <DialogHeader><DialogTitle>Registrar Nuevo Guardia</DialogTitle><DialogDescription>Añade los datos del elemento operativo.</DialogDescription></DialogHeader>
