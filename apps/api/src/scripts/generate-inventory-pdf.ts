...
async function main() {
  console.log('Consultando base de datos para generar el inventario...');
  
  // 1. Obtener resumen de inventario por artículo
  ...
  const outputPath = path.join(__dirname, '../../../../inventario_almacen.pdf');
  ...
  for (const articulo of ARTICULOS_CATALOGO) {
    ...
    // Calcular altura necesaria para la tabla de este artículo
    // Título (15) + Header (18) + Filas (items.length * 16) + Margen final (15)
    ...
    // Título de la sección de artículo
    ...
    currentY += 12; // Spacing entre tablas de artículos
  }
  ...
  // Esperar a que el stream finalice de escribir el archivo
