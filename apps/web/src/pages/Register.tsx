...
export default function Register() {
  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center h-[50vh] gap-3">
        ...
        <p className="text-sm text-muted-foreground">Solo los administradores pueden crear usuarios.</p>
      </div>
    );
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      const user = await apiFetch<{ username: string; role: string }>('/api/auth/register', {
        method: 'POST',
    ...
    <div className="max-w-md mx-auto space-y-6 animate-in fade-in duration-300">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          El nuevo usuario podrá acceder al sistema con las credenciales que indiques.
        </p>
      </div>
      ...
      >
        ...
        <div className="flex gap-3 pt-1">
          <button
            type="button"
            onClick={() => navigate('/')}
            className="flex-1 py-2.5 px-4 text-sm font-medium border border-border rounded-lg hover:bg-muted transition"
          >
            Cancelar
          </button>
          <button
            type="submit"
            disabled={loading}
            className="flex-1 py-2.5 px-4 text-sm font-semibold text-white rounded-lg bg-primary hover:bg-primary/90 disabled:opacity-60 transition"
          >
