...
export default function Inventario() {
  const { data: inventario, isLoading } = useQuery({
    queryKey: ['inventario'],
    queryFn: async () => {
      return apiFetch<any[]>('/api/inventario');
    }
  });
  ...
  const stockBajoItems = (inventario as any[]).filter((i: any) => i.stockBajo).length;

  const totales = (inventario as any[]).reduce((acc: any, curr: any) => ({
  ...
  const handleExportCSV = () => {
    ...
    const rows = (inventario as any[]).map((i: any) => [
      i.articulo, i.totalEntradas, i.almacenNuevo, i.almacenUsado, i.almacenInutilizable, i.almacen, i.enCampo, i.enBajas, i.definitivos, i.perdidas, i.totalExistente
    ]);
  ...
  const handlePrint = () => {
    ...
    const rowsHtml = (inventario as any[]).map((item: any) => `
    ...
    const stockAlertHtml = stockBajoItems > 0
      ? `<div class="alert">&#9651; Stock bajo detectado: ${stockBajoItems} artículo${stockBajoItems > 1 ? 's tienen' : ' tiene'} 5 piezas o menos en almacén. Considera reabastecer.</div>`
      : '';
...
<head>
  <meta charset="UTF-8"/>
  <title>Inventario General — U3 Seguridad Privada</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .summary-card { border: 1.5px solid #e5e7eb; border-radius: 6px; padding: 10px 14px; text-align: center; }
    .summary-card .value { font-size: 22px; font-weight: 900; }
    .summary-card .label { font-size: 9px; color: #6b7280; text-transform: uppercase; font-weight: 700; margin-top: 2px; }
    .section-title { font-size: 12px; font-weight: 700; color: #1d4ed8; margin: 14px 0 6px 0; border-bottom: 1px solid #e5e7eb; padding-bottom: 4px; }
    table { width: 100%; border-collapse: collapse; margin-bottom: 10px; }
    thead tr { background: #1d4ed8; color: #fff; }
    th { padding: 6px 8px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.4px; text-align: left; }
...
<body>
  ...
  <div class="summary-grid">
    <div class="summary-card">
      <div class="value" style="color:#059669">${totales.almacen}</div>
      <div class="label">En Almacén</div>
    </div>
    <div class="summary-card">
      <div class="value" style="color:#2563eb">${totales.enCampo}</div>
      <div class="label">En Campo</div>
    </div>
    <div class="summary-card">
      <div class="value" style="color:#f59e0b">${totales.enBajas}</div>
      <div class="label">En Proceso de Baja</div>
    </div>
    <div class="summary-card">
      <div class="value" style="color:#111">${totales.totalExistente}</div>
      <div class="label">Total Existente</div>
    </div>
  </div>

  ${stockAlertHtml}

  <div class="section-title">Estado actual del inventario por artículo</div>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th class="right">Alm. Nuevo</th>
        <th class="right">Alm. Usado</th>
        <th class="right">Alm. Inútil</th>
        <th class="right">Alm. Total</th>
        <th class="right">En Campo</th>
        <th class="right">En Bajas</th>
        <th class="right">Entregados Def.</th>
    ...
    <tbody>
      ...
      <tr class="totals-row">
        <td>TOTALES</td>
        <td class="right">${totales.totalEntradas}</td>
        <td class="right">${totales.almacenNuevo}</td>
        <td class="right">${totales.almacenUsado}</td>
        <td class="right">${totales.almacenInutilizable}</td>
        <td class="right">${totales.almacen}</td>
        <td class="right">${totales.enCampo > 0 ? totales.enCampo : '—'}</td>
        <td class="right">${totales.enBajas > 0 ? totales.enBajas : '—'}</td>
        <td class="right">${totales.definitivos > 0 ? totales.definitivos : '—'}</td>
        <td class="right">${totales.perdidas > 0 ? totales.perdidas : '—'}</td>
        <td class="right">${totales.totalExistente}</td>
      </tr>
    </tbody>
  </table>
  ...
  <div class="firmas">
    <div class="firma">
      ...
      <small>U3 Seguridad Privada</small>
    </div>
    <div class="firma">
      <div class="firma-line"></div>
      <p>Supervisor / Jefe de Operaciones</p>
      <small>U3 Seguridad Privada</small>
    </div>
  </div>
  ...
  const handleExportPDF = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/inventario/export-pdf`, {
        headers: {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Estado actual del almacén, equipo en uso y bajas definitivas
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={handlePrint}>
            <Printer className="w-4 h-4 mr-1.5" /> Imprimir Vista
          </Button>
        </div>
      </div>
        ...
        <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-xl flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm mt-0.5">
              {stockBajoItems} {stockBajoItems === 1 ? 'artículo tiene' : 'artículos tienen'} 5 piezas o menos en almacén. Considera reabastecer.
            </p>
          </div>
        </div>
      )}
      ...
      <div id="inventario-print-area">
        <div className="hidden" id="print-header-only">
          <h1 style={{ fontSize: '18px', fontWeight: 'bold', marginBottom: '2px' }}>Inventario General</h1>
          <p style={{ fontSize: '12px', color: '#6b7280', marginBottom: '8px' }}>
            Fecha de corte: {fmtDate(new Date())}
          </p>
        </div>
        <div className="rounded-xl border border-border bg-card overflow-hidden shadow-sm">
          <Table>
            ...
            <TableBody>
              {(inventario as any[]).map((item: any) => (
                <TableRow
                  key={item.articulo}
                  className={item.stockBajo ? "bg-amber-50/60 hover:bg-amber-50" : ""}
                >
                  ...
                  <TableCell className="text-right">
                    {item.perdidas > 0
                      ? <span className="text-red-600 font-medium">{item.perdidas}</span>
              ...
              <TableRow className="bg-muted/40 font-bold border-t-2">
                ...
                <TableCell className="text-right">
                  {totales.perdidas > 0
                    ? <span className="text-red-600 font-bold">{totales.perdidas}</span>
                    : '—'
                  }
                </TableCell>
