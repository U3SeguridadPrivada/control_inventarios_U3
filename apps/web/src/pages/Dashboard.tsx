...
export default function Dashboard() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      return apiFetch<{ metrics: any; chartData: any[]; recentMovements: any[] }>('/api/dashboard/metrics');
    }
  });
    ...
    return (
      <div className="flex items-center justify-center h-[50vh] flex-col gap-4">
        ...
        <p className="text-muted-foreground text-center max-w-md text-sm">
          No se pudo cargar la información del dashboard. Verifica que la base de datos PostgreSQL esté activa.
        </p>
      </div>
    );
  }
  ...
  const metricCards = [
    ...
    {
      title: "Total Salidas",
      value: metrics?.totalSalidas ?? 0,
      icon: ArrowUpFromLine,
      color: "text-orange-600",
      bg: "bg-orange-50",
    },
    {
      title: "Items en Campo",
      value: metrics?.itemsEnCampo ?? 0,
      icon: Package,
    ...
    },
    {
      ...
      value: metrics?.guardiasActivos ?? 0,
      icon: Users,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-7 gap-5">
        <Card className="col-span-4">
          <CardHeader className="pb-4">
            <CardTitle className="text-base font-semibold">Actividad de Movimientos</CardTitle>
          </CardHeader>
