...
export default function Usuarios() {
  ...
  const roleMutation = useMutation({
    mutationFn: ({ id, role }: { id: number; role: string }) =>
      apiFetch(`/api/auth/users/${id}`, { method: 'PATCH', body: JSON.stringify({ role }) }),
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['users'] }); setConfirmDelete(null); toast.success('Usuario eliminado'); },
    onError: (e: Error) => toast.error(e.message),
  });

  const pwMutation = useMutation({
    mutationFn: ({ id, password }: { id: number; password: string }) =>
      apiFetch(`/api/auth/users/${id}/password`, { method: 'PATCH', body: JSON.stringify({ password }) }),
    onSuccess: () => { setPwModal(null); setNewPassword(''); toast.success('Contraseña actualizada'); },
    onError: (e: Error) => toast.error(e.message),
  });
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white rounded-lg bg-primary hover:bg-primary/90 transition"
        >
              ...
              {users.map((u) => {
                ...
                return (
                  <tr key={u.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold leading-tight">
                            {u.username}
                            {isMe && <span className="ml-2 text-[10px] text-primary bg-primary/10 px-1.5 py-0.5 rounded-full font-normal">Tú</span>}
                          </p>
                    ...
                    {/* Fecha creación */}
                    <td className="px-5 py-4 text-muted-foreground text-xs whitespace-nowrap">{fmtDate(u.created_at)}</td>

                    {/* Rol */}
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${info.color}`}>
                        ...
                        {!isMe && (
                          <select
                            ...
                            className="text-xs border border-border rounded-md px-2 py-1 bg-background focus:outline-none focus:ring-1 focus:ring-primary cursor-pointer disabled:opacity-50"
                          >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => { setPwModal(u); setNewPassword(''); setShowPw(false); }}
                          className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground hover:bg-muted px-3 py-1.5 rounded-lg transition"
                        >
                        ...
                        {!isMe && (
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 text-xs text-destructive hover:bg-destructive/10 px-3 py-1.5 rounded-lg transition"
                          >
          ...
          <div className="bg-card border border-border rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Usuario: <span className="font-semibold text-foreground">{pwModal.username}</span></p>
              </div>
              <button onClick={() => setPwModal(null)} className="p-1.5 hover:bg-muted rounded-lg transition">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3 py-2 pr-16 text-sm border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary transition"
                  />
                  <button
                    type="button"
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button
                onClick={() => pwMutation.mutate({ id: pwModal.id, password: newPassword })}
                disabled={pwMutation.isPending || newPassword.length < 8}
                className="flex-1 py-2.5 text-sm font-semibold text-white rounded-lg bg-primary hover:bg-primary/90 disabled:opacity-60 transition"
              >
                {pwMutation.isPending ? 'Guardando...' : 'Guardar'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-card border border-border rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className="flex-1 py-2.5 text-sm font-semibold text-white rounded-lg bg-destructive hover:bg-destructive/90 disabled:opacity-60 transition"
              >
