...
const LLAVE_STORAGE = 'u3-machote-ficha-admin-draft';
...
interface Props {
  onVolver?: () => void;
  initialAdministrativoId?: number | string;
  embedded?: boolean;
  fullScreen?: boolean;
  pageMode?: boolean;
  volverUrl?: string;
...
export default function MachoteFichaAdministrativo({
  onVolver,
  initialAdministrativoId,
  embedded = false,
  fullScreen = false,
  pageMode = false,
  volverUrl,
  ...
  const [selectedAdministrativoId, setSelectedAdministrativoId] = useState<string>(
    initialAdministrativoId ? String(initialAdministrativoId) : ''
  );
  ...
  // Consulta de administrativos registrados en la base de datos
  const { data: administrativos = [] } = useQuery({
    queryKey: ['administrativos'],
    queryFn: () => apiFetch<any[]>('/api/administrativos'),
  });

  // Si se pasa initialAdministrativoId o cambia selectedAdministrativoId, cargar datos
  useEffect(() => {
    if (initialAdministrativoId) {
      cargarDatosAdministrativo(String(initialAdministrativoId));
    }
  }, [initialAdministrativoId]);
  ...
  const cargarDatosAdministrativo = async (administrativoId: string) => {
    if (!administrativoId) return;
    setSelectedAdministrativoId(administrativoId);
    const toastId = toast.loading('Cargando expediente del administrativo...');

    try {
      const res = await apiFetch<{ administrativo: any; ficha: FichaState | null }>(
        `/api/administrativos/${administrativoId}/ficha`
      );

      if (res.ficha) {
        // El alta rápida de administrativos (y cualquier ficha_tecnica_json antiguo o
        ...
        // la tabla administrativos — sin esto la ficha se ve con el nombre en blanco.
        if (!f.nombre) f.nombre = (res.administrativo.nombre || '').toUpperCase();
        if (!f.numeroElemento) f.numeroElemento = (res.administrativo.numero_empleado || '').toUpperCase();
        ...
        setFicha(f);
        toast.success(`Ficha técnica cargada: ${res.administrativo.nombre}`, { id: toastId });
      } else {
        // Inicializar con los datos básicos que ya tenga el administrativo en la BD
        const g = res.administrativo;
        const d = desglosarDireccion(g.direccion);
        setFicha({
          ...ESTADO_VACIO,
          nombre: (g.nombre || '').toUpperCase(),
          numeroElemento: (g.numero_empleado || '').toUpperCase(),
          celular: g.telefono || '',
          calleNumero: d.calleNumero,
          colonia: d.colonia,
        ...
        toast.success(`Administrativo seleccionado: ${g.nombre} (plantilla en blanco lista para llenar)`, {
          id: toastId,
        });
      }
    } catch {
      // Fallback local si la llamada falla
      const g = administrativos.find((item) => String(item.id) === String(administrativoId));
      if (g) {
        const d = desglosarDireccion(g.direccion);
        setFicha({
          ...ESTADO_VACIO,
        ...
        toast.success(`Administrativo seleccionado: ${g.nombre}`, { id: toastId });
      } else {
        toast.error('No se pudo cargar la información del administrativo', { id: toastId });
      }
    }
  };

  // Guardar en la base de datos (expediente del administrativo)
  const guardarEnExpediente = async () => {
    if (!selectedAdministrativoId) {
      toast.error('Por favor selecciona primero un administrativo para guardar su ficha en su expediente');
      return;
    }
    ...
    try {
      const res = await apiFetch<{ ok: boolean; administrativo: any }>(
        `/api/administrativos/${selectedAdministrativoId}/ficha`,
        {
          method: 'PUT',
      ...
      if (res.ok) {
        ...
        queryClient.invalidateQueries({ queryKey: ['administrativo-ficha', selectedAdministrativoId] });
        queryClient.invalidateQueries({ queryKey: ['administrativo-documentos'] });
        toast.success(`Ficha técnica guardada exitosamente en el expediente de ${res.administrativo?.nombre}`, {
          id: toastId,
        });
  ...
  const descargarPdfServidor = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/administrativos/ficha-pdf', {
        method: 'POST',
        headers: {
      ...
      const safeName = ficha.numeroElemento || (ficha.nombre ? ficha.nombre.replace(/[^a-zA-Z0-9]/g, '_') : 'administrativo');
      link.href = url;
  ...
  const selectedAdministrativoNombre = administrativos.find((g: any) => String(g.id) === String(selectedAdministrativoId))?.nombre;
      ...
      {pageMode ? (
        <div className="bg-card border border-border rounded-xl p-3 sm:p-4 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 print:hidden">
          <div className="flex items-center gap-3">
            <Link href={volverUrl || (selectedAdministrativoId ? `/administrativos/${selectedAdministrativoId}` : "/administrativos")}>
            ...
            <div>
              <h1 className="text-base sm:text-lg font-bold tracking-tight text-foreground flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-primary" /> Ficha Técnica Oficial — {selectedAdministrativoNombre || ficha.nombre || 'Administrativo'}
              </h1>
          ...
          <div className="flex items-center gap-2 self-start md:self-auto">
            <span className="text-xs font-mono font-bold uppercase px-2.5 py-1 rounded bg-primary/10 text-primary border border-primary/20">
              EXP-FT-{selectedAdministrativoId || 'NUEVO'}
            </span>
          ...
          <div className="flex flex-wrap items-center gap-2 justify-end">
            ...
            <Button
              ...
              disabled={guardandoDb || !selectedAdministrativoId}
          ...
          <div className="flex items-center gap-3 min-w-0">
            {(onClose || onVolver) && (
              <Button
                ...
                title="Regresar al perfil del administrativo"
              >
                <ArrowLeft className="w-4 h-4 text-primary" />
                {selectedAdministrativoNombre || ficha.nombre
                  ? `Volver al Perfil`
                  : 'Volver al Perfil del Administrativo'}
              </Button>
            )}
            <div className="h-4 w-px bg-slate-800 hidden sm:block" />
            <span className="text-xs sm:text-sm font-bold text-white truncate flex items-center gap-2">
              <Users className="w-4 h-4 text-primary" />
              Ficha Técnica — {selectedAdministrativoNombre || ficha.nombre || 'Administrativo'}
            </span>
        ...
        <div className="rounded-xl border border-border bg-card p-4 shadow-sm space-y-3 print:hidden">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold tracking-tight flex items-center gap-1.5">
                Ficha Técnica Oficial de Administrativo
              </span>
              {selectedAdministrativoNombre && (
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                disabled={guardandoDb || !selectedAdministrativoId}
                onClick={guardarEnExpediente}
                title={selectedAdministrativoId ? 'Guarda los datos permanentemente en el expediente del administrativo' : 'Selecciona un administrativo para guardar'}
              >
                {guardandoDb ? (
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-border text-xs">
            {/* Cargar administrativo existente */}
            <div className="flex items-center gap-1.5 min-w-[240px]">
              ...
              <select
                ...
                value={selectedAdministrativoId}
                onChange={(e) => cargarDatosAdministrativo(e.target.value)}
              >
                <option value="">
                  {selectedAdministrativoId ? '-- Cambiar de administrativo --' : 'Seleccionar administrativo para su expediente...'}
                </option>
                {administrativos.map((g: any) => (
                  <option key={g.id} value={g.id}>
                    {g.nombre} · {g.numero_empleado}
                  </option>
                ))}
              </select>
            </div>
                ...
                >
                  {ficha.fotoUrl ? (
                    <>
                      <img
                        src={ficha.fotoUrl}
                        alt="Fotografía del Administrativo"
